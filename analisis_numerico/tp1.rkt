...
(define (main)
  (display "Ingrese un valor para el radio del tanque (entre 1.95m y 40m): ")
  (let ((radio (read)))
    (if (and (> radio 1.95)
	     (< radio 40))
	(begin (display "En un tanque con el radio deseado, para qeu contenga 30m^3, debe llenarse a una profundidad de ")
	       (display (nr-secante (funcion radio)
				  (raiz-biseccion (funcion radio) 0 4)
				  -1))
               (newline))
	(begin (display "El nro de R ingresado es invalido.")
	       (newline)
               (main)))))
